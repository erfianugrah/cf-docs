...
local function pagebreak_from_config(config)
	...
	local html_class = config["html-class"] and stringify(config["html-class"])
	...
	local odt_style = config["odt-style"] and stringify(config["odt-style"]) or os.getenv("PANDOC_PAGEBREAK_ODT_STYLE")
...
local function para_pagebreak(raw_pagebreak, checks)
	local is_pb = function(para)
		return checks:find_if(function(pred)
			return pred(para)
		end)
	end
...
function Pandoc(doc)
	...
	return doc:walk({
		...
		Para = #paragraph_checks > 0 and para_pagebreak(raw_pagebreak, paragraph_checks) or nil,
	})
end
